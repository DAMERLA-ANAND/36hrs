...
import { AlertCircle, Gem, Plus, RefreshCw, Rocket } from 'lucide-react';
...
function ErrorCard({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <Card className='col-span-full flex flex-col items-center justify-center border-destructive/50 bg-destructive/5 p-8 rounded-xl'>
...
function Interviews() {
  const { interviews, interviewsLoading, error: interviewsError, refetch: refetchInterviews } = useInterviews();
  ...
  const handleRetry = () => {
    setError(null);
    fetchOrganizationData();
    if (refetchInterviews) {
      refetchInterviews();
    }
  };

  const displayError = error || interviewsError;
